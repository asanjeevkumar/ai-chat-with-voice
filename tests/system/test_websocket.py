...
async def test_websocket_message_flow(client):
    ...
    with patch(
        "core.llm.service.LLMService.get_completion", return_value="Test response"
    ), patch(
        "core.tts.service.TTSService.text_to_speech", return_value=b"mock audio data"
    ):

        with client.websocket_connect("/api/chat/stream") as websocket:
            # Send message
            websocket.send_text(json.dumps({"type": "text", "content": "Test message"}))
            ...
            assert (
                audio_data["content"] == "bW9jayBhdWRpbyBkYXRh"
            )  # base64 of "mock audio data"
...
async def test_websocket_error_handling(client):
    ...
    with patch(
        "core.llm.service.LLMService.get_completion",
